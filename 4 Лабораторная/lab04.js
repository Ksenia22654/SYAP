...
function sortStudents()// сортируеи студентов по возрастанию
{
  let arrayStudent=Array.from(students);
  arrayStudent.sort((a,b)=>a.studentId-b.studentId);
  students=new Set(arrayStudent)
  //Если a.studentId меньше b.studentId, функция возвращает отрицательное значение, и a помещается перед b.
  //Если a.studentId больше b.studentId, функция возвращает положительное значение, и a помещается после b.
  //Если a.studentId равно b.studentId, функция возвращает ноль, и порядок этих двух элементов не изменяется.
}
let student1 = {
  studentId: 123,
  group: "Группа 1",
